...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', seg_cfg="segheads.yaml", ch=3, nc=None, segnc=20, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(cfg, dict):
            ...
            #self.out_idx = [31, 40]  # model output index
            #self.det_idx = 31
        if len(self.yaml['backbone']) == 10:
            self.out_idx = [24, 33]  # model output index
            self.det_idx = 24
        #elif len(self.yaml['backbone']) == 14:
            #self.out_idx = [30, 39]  # model output index
            #self.det_idx = 30
        elif len(self.yaml['backbone']) == 51:
            self.out_idx = [105, 114]  # model output index
            self.det_idx = 105
        else:
            print("Only support yolov5* and yolov5*6!")
            #assert (len(self.yaml['backbone']) in [10,12]) #hanfujun
            assert (len(self.yaml['backbone']) in [10,80])
        self.segnc = segnc

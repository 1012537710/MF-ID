...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', seg_cfg="segheads.yaml", ch=3, nc=None, segnc=20, anchors=None):  # model, input channels, number of classes
        ...
        if len(self.yaml['backbone']) == 10:
            ...
            self.det_idx = 24
        elif len(self.yaml['backbone']) == 13:
            self.out_idx = [30, 39]  # model output index
            self.det_idx = 30
        else:
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            if type(m) is RepVGGBlock:  ### hanfujun
                m.switch_to_deploy()  ### hanfujun
        self.info()
        return self
...
def parse_model(d, sd,ch):  # model_dict, segmentation dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head'] + sd['SegHead']):  # from, number, module, args
        ...
        n = n_ = max(round(n * gd), 1) if n > 1 else n  # depth gain
                 ...
                 #BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, BoT3, CBAM,MobileNetV3_InvertedResidual,ShuffleNetV2_InvertedResidual,conv_bn_hswish,
                 #stem, MBConvBlock, DepthSepConv, Conv_maxpool]:
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3STR, C3SPP, C3Ghost, CBAM, Conv_maxpool, ShuffleNetV2_InvertedResidual,
                 conv_bn_hswish, MobileNetV3_InvertedResidual, stem, MBConvBlock, DepthSepConv, RepVGGBlock, HorBlock, 
                 C3HB, C3SE, C3_SE_Attention, C3CBAM, C3CA,DepthWiseConv1,TransformerBlock,ASPP,ECA]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8) 

            args = [c1, c2, *args[1:]]
            if m in [CoT3, BottleneckCSP, C3, C3TR, C3Ghost, BoT3, HorBlock, C3HB, C3SE, C3_SE_Attention, C3CBAM, C3CA]:
            ...
            c2 = args[0]
        elif m is CoordAtt:   ##by hanfujun
            inp, oup = ch[f], args[0] ##by hanfujun
            if oup!= no: ##by hanfujun
               oup = make_divisible(oup * gw, 8) ##by hanfujun
            args = [inp,oup,*args[1:]] ##by hanfujun
        elif m is SCConv:   ##by hanfujun
            inplanes, planes = ch[f], args[0] ##by hanfujun
            if planes!= no: ##by hanfujun
               planes = make_divisible(planes * gw, 8) ##by hanfujun
            args = [inplanes,planes,*args[1:]] ##by hanfujun
        else:
            c2 = ch[f]

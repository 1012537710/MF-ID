...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            if not opt.noautoanchor:
                # 这里是以前没有的 为了并卡训练所以才加的 hanfujun
                if hasattr(model, 'module'):
                    model = model.module
                # 这里是以前没有的 为了并卡训练所以才加的 hanfujun
                    #m = model.module.model[model.det_idx]
    ...
    if hasattr(model, 'module'): # hanfujun
         model = model.module # hanfujun
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # print("seg_pred:",type(seg_pred),len(seg_pred),type(seg_pred[0]), [p.shape for p in seg_pred[0]])
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default=ROOT / '/root/YePeng/P3ADA/runs/Normal-->Foggy/Edgeseg(1%Gabor)/', help='save to project/name')
